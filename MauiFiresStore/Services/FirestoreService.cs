...
using System;
using MauiFiresStore.Models;

namespace MauiFiresStore.Services;
...
public class FirestoreService
{
    ...
    private async Task SetupFireStore()
    {
        if (db == null)
        {
            var stream = await FileSystem.OpenAppPackageFileAsync("duang212-19983-firebase-adminsdk-xw4go-c14b017ac7.json");
            var reader = new StreamReader(stream);
            var contents = reader.ReadToEnd();
            db = new FirestoreDbBuilder
            {
                ProjectId = "duang212-19983",
